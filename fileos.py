            ...
            if ok_pressed and new_name != current_name:
                ...
                try:
                    ...
                    if not is_directory and new_name.split('.')[-1] != current_name.split('.')[-1]:
                        ...
                        os.remove(old_path)
                        item.setText(new_name)
                        item.setIcon(ui1.get_file_icon(new_path))
                    else:
                        # Rename files or directories
                        os.rename(old_path, new_path)
                        item.setText(new_name)
...
class DisposeTab3(QtWidgets.QTabWidget):
    ...
    def list(self):     #打开文件夹
        ...
        # 设置大图标视图的图标大小和显示模式
        icon_size = QtCore.QSize(130, 130)  # 设置图标大小
        ui2.listWidget2.setIconSize(icon_size)

        ui2.listWidget2.setViewMode(QtWidgets.QListWidget.IconMode)

        # 设置文件图标之间的间隔
        ui2.listWidget2.setSpacing(50)  # 这里的10可以根据需要调整
            ...
            if ok_pressed and new_name != current_name:
                ...
                try:
                    ...
                    if not is_directory and new_name.split('.')[-1] != current_name.split('.')[-1]:
                        ...
                        os.remove(old_path)
                        item.setText(new_name)
                        item.setIcon(ui2.get_file_icon(new_path))
                    else:
                        # Rename files or directories
                        os.rename(old_path, new_path)
                        item.setText(new_name)
